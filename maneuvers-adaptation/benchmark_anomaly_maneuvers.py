    ...
    for i, (true, md) in enumerate(tqdm(dataset)):
        ...
        for acc_id, (simple_threshold, opt_metric, scores) in enumerate(
            ...
            if i >= min(len(p) for p in all_model_preds):
                # This would mean that the current model evaluated has an higher index
                # than the amount of predictions made during the train period 
                # (one for each model)
                break
...
def main():
    ...
    residues_dir = "data/residues"
    ## Build Merlion Custom Dataset
    dataset = CustomAnomalyDataset(
        rootdir=residues_dir,   # where the data is stored
