### A try to infere the models trained by maneuver-detection.py
### FIXME
...
def main():
    ...
    for model_type in tqdm(md.model_types):
        ...
        model = ModelFactory.create(name=model_type)

        model_type_save_dir = join(rootdir, 'results', 'models', model_type)
        max_combi = max([int(f) for f in os.listdir(model_type_save_dir) if os.path.isdir(os.path.join(model_type_save_dir, f))])
        
        # Iterate over each combination
        for i, combination in enumerate(tqdm(hyperparam_combinations), start=1):
            if i > max_combi:
                print(f'Reached last saved model {max_combi}')
                break
            
            ## Load the saved model
            save_dir = join(rootdir, 'results', 'models', model_type, str(i))
            loaded_model = type(model).load(dirname=save_dir)
            ...
            concatenated_train_df, concatenated_test_df, concatenated_test_labels_df = md.get_concat_ts()
            
            test_ts = TimeSeries.from_pd(concatenated_test_df)
            test_labels_ts = TimeSeries.from_pd(concatenated_test_labels_df)
            
            # Evaluate model
            print('Infering the saved model...')
            labels = loaded_model.get_anomaly_label(test_ts)
